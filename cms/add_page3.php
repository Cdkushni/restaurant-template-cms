...
if ($section == "Add Tertiary Page") {
	...
	if ($submit == "Save") {
		...
		if(!$queryTEST){
			...
			$tbl = 'CREATE TABLE nav3_'.$reference.'( '.
		...
		$refid = create_reference($db,3001,3999);
	...
	if (!$saved) {
		...
		while($lvl = mysql_fetch_array($lvlquery)){
			...
			while($lvl2 = mysql_fetch_array($lvl2query)){
				...
				echo "<option value='" .$lvlref. $lvl2ref. "'>" .$lvl2name. "</option>";

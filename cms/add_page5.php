...
if ($section == "Add Fifth Level Page") {
	...
	if ($submit == "Save") {
		...
		if(!$queryTEST){
			...
			$tbl = 'CREATE TABLE nav5_'.$reference.'( '.
		...
		$refid = create_reference($db,5001,5999);
		
		
		$sql = "INSERT INTO $db (id, name, page, type, page_title, meta_title, meta_description, meta_keywords, content, sidebar, image, url, urltarget, showhide, deleteable, reference) VALUES ('$num_insert', '$name', '$pagename', '$type', '$page_title', '$meta_title', '$meta_description', '$meta_keywords', '$content', '$sidebar', '$image', '$url', '$urltarget', '$showhide', '$deleteable', '$refid');";
	...
	if (!$saved) {
		...
		// IMPORTANT -->
		important("<p><b>Button Text</b>: The text that will appear in the navigation. The navigation will automatically be created with this name.</p>");
		// -->
		...
		$lvlquery = mysql_query("SELECT * FROM nav1 WHERE reference != 1001");
		while($lvl = mysql_fetch_array($lvlquery)){
			$lvlname = $lvl['name'];
			$lvlref = $lvl['reference'];
			
			
			$lvl2query = mysql_query("SELECT * FROM nav2_$lvlref ORDER BY id");
			while($lvl2 = mysql_fetch_array($lvl2query)){
				$lvl2name = $lvl2['name'];
				$lvl2ref = $lvl2['reference'];
				
			
				$lvl3query = mysql_query("SELECT * FROM nav3_$lvlref$lvl2ref ORDER BY id");
				while($lvl3 = mysql_fetch_array($lvl3query)){
					$lvl3name = $lvl3['name'];
					$lvl3ref = $lvl3['reference'];
					
					echo "<optgroup label='" .$lvl3name. "'>";
					
					
					$lvl4query = mysql_query("SELECT * FROM nav4_$lvlref$lvl2ref$lvl3ref ORDER BY id");
					while($lvl4 = mysql_fetch_array($lvl4query)){
						$lvl4name = $lvl4['name'];
						$lvl4ref = $lvl4['reference'];
						
						echo "<option value='" .$lvlref. $lvl2ref. $lvl3ref. $lvl4ref. "'>" .$lvl4name. "</option>";

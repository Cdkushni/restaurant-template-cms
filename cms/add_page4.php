...
if ($section == "Add Fourth Level Page") {
	...
	if ($submit == "Save") {
		...
		if(!$queryTEST){
			...
			$tbl = 'CREATE TABLE nav4_'.$reference.'( '.
		...
		$refid = create_reference($db,4001,4999);
		...
		while($lvl = mysql_fetch_array($lvlquery)){
			...
			while($lvl2 = mysql_fetch_array($lvl2query)){
				...
				while($lvl3 = mysql_fetch_array($lvl3query)){
					...
					echo "<option value='" .$lvlref. $lvl2ref. $lvl3ref. "'>" .$lvl3name. "</option>";

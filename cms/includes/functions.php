...
function mysql_escape(){
	// we need linebreaks to remain	
	$lb = "
";	
	$linebreaks1 = array("\\r\\n", "\\r", "\\n");
	$linebreaks2 = array($lb, $lb, $lb);
	// -->
		...
		foreach($_REQUEST AS $key => $data){
			if (is_array($_REQUEST[$key])){
				foreach($_REQUEST[$key] AS $iKey => $iData){
					if (get_magic_quotes_gpc()) { $_REQUEST[$key][$iKey] = stripslashes($_REQUEST[$key][$iKey]); }
					$_REQUEST[$key][$iKey] = mysql_real_escape_string($_REQUEST[$key][$iKey]);	
				}
			}else{	
				if (get_magic_quotes_gpc()) { $_REQUEST[$key] = stripslashes($_REQUEST[$key]); }
				$_REQUEST[$key] = str_replace($linebreaks1, $linebreaks2 ,mysql_real_escape_string($_REQUEST[$key]));
			}
		}
	}
	...
	if (count($_GET)>0){
		foreach($_GET AS $key => $data){
			if (is_array($_GET[$key])){
				foreach($_GET[$key] AS $iKey => $iData){
					if (get_magic_quotes_gpc()) { $_GET[$key][$iKey] = stripslashes($_GET[$key][$iKey]); }
					$_GET[$key][$iKey] = mysql_real_escape_string($_GET[$key][$iKey]);	
				}
			}else{	
				if (get_magic_quotes_gpc()) { $_GET[$key] = stripslashes($_GET[$key]); }
				$_GET[$key] = str_replace($linebreaks1, $linebreaks2 ,mysql_real_escape_string($_GET[$key]));
			}
		}
	}
	...
	if (count($_POST)>0){
		foreach($_POST AS $key => $data){
			if (is_array($_POST[$key])){
				foreach($_POST[$key] AS $iKey => $iData){
					if (get_magic_quotes_gpc()) { $_POST[$key][$iKey] = stripslashes($_POST[$key][$iKey]); }
					$_POST[$key][$iKey] = mysql_real_escape_string($_POST[$key][$iKey]);	
				}
			}else{	
				if (get_magic_quotes_gpc()) { $_POST[$key] = stripslashes($_POST[$key]); }
				$_POST[$key] = str_replace($linebreaks1, $linebreaks2 ,mysql_real_escape_string($_POST[$key]));
			}
		}
	}
}
...
function alert ($text, $TF) {
	echo "<div style='width:600px;'>";
		if ($TF) {
			echo "<div id='alert_success' class='clearFix'><p><b>Success!</b></p></div>";
		} else {
			echo "<div id='alert_error' class='clearFix'><p><b>Error!</b></p></div>";
		}
...
function show_statistics ($p) {
	...
	if($GA_email!='' && $GA_password!=''){
		...
		try{
			...
			echo "<p><b>Page Statistics From: " .date("F d, Y", strtotime($start_date)) ." to " .date("F d, Y", strtotime($end_date)) ."</b></p>";
